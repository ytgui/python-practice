...
class Scene:
    ...
    def render(self):
        ...
        pixels = [[Color() for _ in range(self.width)] for _ in range(self.height)]
    ...
    def _trace_ray(self, ray, depth=0, max_depth=5):
        ...
        color += self._trace_ray(reflected_ray, depth + 1) * obj.material.specular
        '''
        return color
...
class Material:
    ...
    def __init__(self, color, specular=0.5, lambert=1.0, ambient=0.2):
        self.color = color
        self.specular = specular  # 镜面
        self.lambert = lambert  # 亮度
        self.ambient = ambient  # 环境
...
def main():
    # objects
    objects = [Sphere(Point(150, 120, -20), 80, Material(Color(0xFF, 0, 0), specular=0.2)),
               Sphere(Point(420, 120, 0), 100, Material(Color(0, 0, 0xFF), specular=0.8)),
               Sphere(Point(320, 240, -40), 50, Material(Color(0, 0xFF, 0))),
               Sphere(Point(300, 200, 200), 100, Material(Color(0xFF, 0xFF, 0), specular=0.8)),
               Sphere(Point(300, 130, 100), 40, Material(Color(0xFF, 0, 0xFF))),
               Sphere(Point(300, 1000, 0), 700, Material(Color(0xFF, 0xFF, 0xFF),lambert=0.5))]

    # light and camera
    lights = [Point(200, -100, 0), Point(600, 200, -200)]
    # camera = Point(200, 200, -400)
    camera = Point(200, 200, -800)

    # ready rendering
    scene = Scene(camera, objects, lights, width=600, height=400)
    pixels = scene.render()
